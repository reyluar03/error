...
screen -dmS proxy python /etc/SSHPlus/proxy.py 8010
sed -i '/proxy.py/d' /etc/autostart
echo -e "netstat -tlpn | grep -w 8010 > /dev/null || screen -dmS proxy python /etc/SSHPlus/proxy.py 8010" >>/etc/autostart

/bin/cat <<"EOM" >/etc/autostart
#!/bin/bash
clear
#INICIO AUTOMATICO
netstat -tlpn | grep -w 8010 > /dev/null || screen -dmS proxy python /etc/SSHPlus/proxy.py 8010
EOM
...
BUFLEN = 8196 * 8
TIMEOUT = 60
MSG = 'GTM NO LOAD Powered by: FFAST-OWNER Richard Caritos Verdadero'
