...
$query2 = $mysqli->query("SELECT * FROM users WHERE ".$is_validated." OR ".$freeze." OR  ".$premium_deactived ." AND ".$vip_deactived." AND ".$private_deactived."
");
if($query2->num_rows > 0)
{
	while($row2 = $query2->fetch_assoc())
	{
		...
		$data2 .= 'pkill -u '.$toadd.' && userdel -rf '.$toadd.''.PHP_EOL;
	}
}
